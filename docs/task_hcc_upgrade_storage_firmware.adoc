...
[role="lead"]
A partire da Element 12,0 e dalla versione 2,14 dei servizi di gestione, puoi eseguire aggiornamenti solo firmware sui nodi di storage H-Series utilizzando l'interfaccia utente di controllo cloud ibrido NetApp e l'API REST. Questa procedura non aggiorna il software Element e consente di aggiornare il firmware dello storage al di fuori di una release elemento principale.
...
IMPORTANT: Per i nodi di storage H610S che eseguono il software Element versione 12.0, applicare D-patch SUST-909 prima di eseguire l'aggiornamento al bundle firmware di storage 2.27. Contattare il supporto NetApp per ottenere la D-patch prima di eseguire l'aggiornamento. Vedere link:rn_storage_firmware_2.27.html["Note di rilascio del bundle del firmware di archiviazione 2,27"].


IMPORTANT: Prima di aggiornare il firmware sui nodi di storage, è necessario eseguire l'aggiornamento al bundle di servizi di gestione più recente. Se si sta aggiornando il software Element alla versione 12,2, per procedere sono necessari i servizi di gestione 2.14.60 o successivi.
...
NOTE: Per una matrice completa del firmware e del firmware del driver per l'hardware in uso, vedere link:firmware_driver_versions.html["Versioni firmware supportate per i nodi di storage NetApp HCI"].
...
.Di cosa hai bisogno
* Se il nodo di gestione non è connesso a Internet, si dispone di https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab/download/62654/Storage_Firmware_Bundle["scaricato il pacchetto firmware dello storage"^].
...
TIP: Il processo di upgrade richiede circa 30 minuti per nodo.
...
.Fasi
. Scaricare il pacchetto di aggiornamento del firmware di storage più recente su un dispositivo accessibile al nodo di gestione; accedere al https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab/download/62654/Storage_Firmware_Bundle["Pagina bundle firmware storage software Element"^] e scaricare l'immagine del firmware di storage più recente.
